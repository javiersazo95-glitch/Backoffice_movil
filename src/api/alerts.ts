...
export async function getAlerts(search?: string, severity?: AlertSeverity, page = 0, size = 15): Promise<PageResponse<AlertResponse>> {
  const response = await apiClient.get<PageResponse<AlertResponse>>('/alerts', { params: { search, severity, page, size } });
